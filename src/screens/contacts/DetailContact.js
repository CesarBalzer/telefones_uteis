...
import HeaderDetailContact from '../../components/Headers/HeaderDetailContact';
...
const DetailContact = ({ route }) => {
    ...
    navigate.setOptions({
      headerLeft: () => (
        <TouchableOpacity onPress={() => navigate.goBack()}>
          ...
          >
            <Icon name={'chevron-left'} size={28} color={activeColors.text} />
            <Text style={{ color: activeColors.text }}>Voltar</Text>
          </View>
        </TouchableOpacity>
      )
    });
  ...
  const renderSection = (icon, title, content) => (
    <View style={styles.section}>
      <View style={styles.sectionHeader}>
        <Icon name={icon} size={24} color={activeColors.accent} />
  ...
  const createDirectCallShortcut = async (number) => {
    try {
      ...
      console.log('Failed to create direct call shortcut:', error);
    }
  };
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      />
      <View style={{ backgroundColor: activeColors.primary, marginTop: 10 }}>
